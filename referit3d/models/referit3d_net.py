...
class ReferIt3DNet_transformer(nn.Module):
    ...
    def __init__(self,
        ...
        self.refer_encoder = nn.TransformerDecoder(torch.nn.TransformerDecoderLayer(d_model=self.inner_dim, nhead=decoder_nhead_num, dim_feedforward=2048, activation="gelu"), num_layers=decoder_layer_num)

        # Classifier heads
        self.language_clf = nn.Sequential(nn.Linear(self.inner_dim, self.inner_dim), nn.ReLU(), nn.Dropout(self.drop_rate), nn.Linear(self.inner_dim, n_obj_classes))
        self.object_language_clf = nn.Sequential(nn.Linear(self.inner_dim, self.inner_dim), nn.ReLU(), nn.Dropout(self.drop_rate), nn.Linear(self.inner_dim, 1))

        if not self.label_lang_sup:
            self.obj_clf = MLP(self.inner_dim, [512, 512, n_obj_classes], dropout_rate=self.drop_rate)

        self.obj_feature_mapping = nn.Sequential(
            nn.Linear(512, self.inner_dim),
    ...
    def compute_loss(self, batch, CLASS_LOGITS, LANG_LOGITS, LOGITS, AUX_LOGITS=None):
        ...
        total_loss = referential_loss + 0.5 * obj_clf_loss + 0.5 * lang_clf_loss
        return total_loss
